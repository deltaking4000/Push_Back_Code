...
void SuperSoloAWP() {
  ...
  chassis.pid_wait();
  spin_outtake();
  pros::delay(1*1050); // score into long goal
  lift_matchloader();
  ...
  chassis.pid_wait_quick_chain();
  chassis.pid_odom_set(40.5_in, DRIVE_SPEED_MAX); 
  chassis.pid_wait_quick_chain();
  chassis.pid_turn_set(90_deg, TURN_SPEED);
  chassis.pid_wait_quick_chain();
  spin_intake();
  ...
  //pros::delay(0.2*1000); // score into long goal
  reverse_intake();
  pros::delay(0.1*1000); // score into long goal
  spin_intake();
  pros::delay(2*1000); // score into long goal // score long goal
...
void SplitGoalRight() {
  ...
  chassis.pid_wait();
  lift_matchloader();
  chassis.pid_odom_set(7_in, DRIVE_SPEED); //drive to low goal
  chassis.pid_wait();
  pros::delay(0.5 * 1000);
  reverse_intake();
  ...
  chassis.pid_wait();
  pros::delay(0.60 * 1000);//enter matchloader and wait. Needs tuning as it picks up 2 blocks too much
...
void SplitGoalLeft() {
  ...
  chassis.odom_xyt_set(46.2_in, 14.8_in, 334_deg);    // Set the current position, you can start at a specific position with this
  ...
  // chassis.pid_wait_quick_chain();
  chassis.pid_odom_set(26_in, DRIVE_SPEED_SLOW); 
  chassis.pid_wait_until(16_in);
  drop_matchloader();
  chassis.pid_wait();
  ...
  */
  ...
  // go to wing
  chassis.pid_odom_set(31.5_in, DRIVE_SPEED); 
  chassis.pid_wait_quick_chain();
  ...
  // wing
  chassis.pid_odom_set(15_in, DRIVE_SPEED_MAX); 
  chassis.pid_wait_quick_chain();
  chassis.pid_wait();
...
void Right_7_ball_Base() {
  ...
  */
  ...
  // drive backward to long goal
  chassis.pid_drive_exit_condition_set(90_ms, 1_in, 150_ms, 3_in, 100_ms, 250_ms);
  chassis.pid_drive_set(-26.5_in, DRIVE_SPEED_SLOW); 
  chassis.pid_wait_quick_chain();
  default_constants();
  // score into long goal
  spin_intake();
